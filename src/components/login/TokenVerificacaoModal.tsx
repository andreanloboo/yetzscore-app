...
interface TokenVerificacaoModalProps {
  /** Código verificado com sucesso → segue para a área logada / próximo passo. */
  onVerified: () => void;
  /** Fecha o fluxo (X). */
  onClose: () => void;
  /** "Voltar" (código expirado). Padrão: onClose. */
  onVoltar?: () => void;
}

// ─── Login / Envio de token (ref Figma 7929:9436) ────────────────────────────
// Aparece logo após o "Entrar" com credenciais válidas. Regra específica do
// login: o código fica inválido assim que qualquer caractere for uma letra
// (diferente do fluxo de recuperação, que filtra não-dígitos).
export default function TokenVerificacaoModal({ onVerified, onClose, onVoltar }: TokenVerificacaoModalProps) {
  const [digits, setDigits] = useState<string[]>(Array(6).fill(""));
  const [helpOpen, setHelpOpen] = useState(false);
  const inputsRef = useRef<(HTMLInputElement | null)[]>([]);
  const { remaining, restart } = useCountdown(59);
  const expired = remaining === 0;
  const masked = maskEmail(MOCK_EMAIL);

  const hasLetter = digits.some((d) => /[a-zA-Z]/.test(d));
  const allFilled = digits.every((d) => d !== "");
  const allDigits = digits.every((d) => /^\d$/.test(d));
  // Inválido com qualquer letra, ou quando preenchido com caractere não numérico.
  const codeError = hasLetter || (allFilled && !allDigits);

  useEffect(() => {
    inputsRef.current[0]?.focus();
  }, []);

  // Código com 6 números → autentica e segue para a área logada.
  useEffect(() => {
    if (allFilled && allDigits) onVerified();
  }, [allFilled, allDigits, onVerified]);
  ...
  function handleChange(index: number, raw: string) {
    // Aceita qualquer caractere (inclusive letras) para que letras apareçam
    // e disparem o estado inválido.
    const v = raw.slice(-1);
    setDigit(index, v);
        ...
        <div className="flex w-full flex-col items-center gap-6">
          ...
          {expired && (
            <FadeSlideIn className="flex w-full flex-col gap-2">
              <button type="button" onClick={onVoltar ?? onClose} className={`${primaryButton} w-full`}>
                Voltar
              </button>
              <button
                type="button"
